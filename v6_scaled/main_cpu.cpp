// CPU Benchmark Runner for V6 Scaled (.cpp files: V1, V2, V3, V5)
#include <iostream>
#include <fstream>
#include <string>
#include <cstdlib>
...
void run_scale(int scale, std::ofstream& out_file) {
    std::string banner = "======================================================================\n"
                         "             V6 CPU SCALED BENCHMARK SUITE (" + std::to_string(scale) + "x" + std::to_string(scale) + ")\n"
                         "======================================================================\n\n";
    std::cout << banner;
    out_file << banner;

    std::string scale_str = std::to_string(scale);
    std::vector<std::pair<std::string, std::string>> cpu_targets = {
        {"V1 Float32 Baseline", "v1_float32_exe"},
        {"V2 Int8 Quantized", "v2_int8_exe"},
        {"V3 Int8 AVX2 SIMD", "v3_simd_exe"},
        {"V5 Transformer Block", "v5_transformer_exe"}
    };

    for (const auto& target : cpu_targets) {
...
int main(int argc, char* argv[]) {
    ...
    std::ofstream out_file("cpu.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not open cpu.txt for writing.\n";
        return 1;
    }

    for (int scale : scales) {
        run_scale(scale, out_file);
    }

    std::string footer = "======================================================================\n"
                         "             V6 CPU BENCHMARKS COMPLETE (Saved to cpu.txt)\n"
                         "======================================================================\n";
    std::cout << footer;
    out_file << footer;
    out_file.close();

    // Also copy cpu.txt to root if running inside v6_scaled
    std::system("cp cpu.txt ../cpu.txt 2>/dev/null");
